...
try_run_python(["python", "src\\removePHI.py", "--filename", os.path.join("..\\", entries), "--type", "entries"])
log(start, timestamp, f"Removed PHI from dataset: {entries}")

# Run removePHI on events
try_run_python(["python", "src\\removePHI.py", "--filename", os.path.join("..\\", events), "--type", "events"])
